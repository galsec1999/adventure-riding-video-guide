#!/usr/bin/env python3
"""Apply the product 3.4.0 version and Chris Birch content-count bump.
...
OLD = "3.3.0"
NEW = "3.4.0"
OLD_REV = "3.3.0-20260806a"
NEW_REV = "3.4.0-20260806a"
TOTAL = 578
FULL = 425
SHORTS = 153
...
def main() -> int:
    ...
    replace(ROOT / "service-worker.js", [
        ("PWA release 3.3.0 — source-verified Shorts recovery and local semantic runtime.",
         "PWA release 3.4.0 — verified Chris Birch Adventure training expansion."),
        (OLD_REV, NEW_REV),
        (OLD, NEW),
    ])
    ...
    manifest["description"] = f"{TOTAL} הדרכות רכיבה פעילות ומסווגות, ובהן {FULL} סרטונים מלאים ו־{SHORTS} קצרים מאומתים, בעברית ובאנגלית."
    ...
    config.update({
        ...
        "document_version": "3.4.0",
        "meta_title_he": f"מדריך הווידאו הקהילתי לרכיבת אדוונצ'ר — גרסה {NEW}",
        "meta_description_he": f"{TOTAL} הדרכות רכיבה פעילות ומסווגות, ובהן {FULL} סרטונים מלאים ו־{SHORTS} קצרים מאומתים, מסלולי לימוד וחיפוש חכם בעברית ובאנגלית.",
        "release_version": NEW,
        ...
        "meta_description_en": f"{TOTAL} active, curated riding tutorials: {FULL} full videos and {SHORTS} verified Shorts, with learning paths and smart search in Hebrew and English.",
    })
    ...
    print(json.dumps({"product_version": NEW, "total_items": TOTAL, "full": FULL, "shorts": SHORTS}, ensure_ascii=False))
    return 0
